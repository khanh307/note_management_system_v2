...
class PriorityScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const _PriorityHome();
  }
}

class _PriorityHome extends StatefulWidget {
  const _PriorityHome({Key? key}) : super(key: key);

  @override
  State<_PriorityHome> createState() => _PriorityHomeState();
}

class _PriorityHomeState extends State<_PriorityHome> {
  final priorityCubit = PriorityCubit(PriorityRepository(email: 'kyle@r2s.com.vn'));
          ...
          listener: (_, state) {
            if (state is SuccessSubmitPriorityState) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Successfully delete priority')));
              priorityCubit.getAllPriorities();
  ...
  Future<void> _showDialog(BuildContext context, PriorityModel? priority) async {
    if (priority != null) {
